...
const {
  rooms, initRooms, getRoom, getAllRooms, getRoomsByMode, findRoomOfUser,
  clearRoomTimers, resetRoom, isFull, allReady, canRevealCode, generateCode,
  formatPersonalCode, banUser, unbanUser, isBanned, createHiddenRoom,
  getHiddenRoom, getAllHiddenRooms, deleteHiddenRoom, ensureRoom, addRoomsToMode,
  removeExtraRoom, restoreRooms, restoreEloData, clearElo, eloData, getElo,
  getFullElo, updateElo, calculateNewElo, registerIGN, estimateWinsToNextTier,
  addRankRoomsToMode, removeRankRoom, getAllRankRooms, getRankRoomsByMode,
  getAllNormalRooms, getNormalRoomsByMode, extractAllKDAResult,
} = require('./src/rooms');
...
async function finalizeRankSessionIfReady(session, room, roomId, kdaMap) {
  if (!session) { console.warn('⚠️ finalizeRankSessionIfReady: session rỗng.'); return null; }
  ...
  if (!room || !room.isRank) {
    if (roomId === 'admin-test') {
      ...
      console.log(`🧪 [admin-test]: dùng fake room mode=${room.mode} để tính ELO.`);
    } else {
      console.warn(`⚠️ [${roomId}]: room không tồn tại hoặc không phải rank.`);
      return null;
    }
  }

  const realPlayers = session.players.filter(([, data]) => !data.isBot).map(([id]) => id);
  if (realPlayers.length === 0) { console.warn(`⚠️ [${roomId}]: không có người chơi thật.`); return null; }
  ...
  if (!ready) {
    console.log(`ℹ️ [${roomId}]: chưa đủ kết quả (${submittedReal.length}/${realPlayers.length}).`);
    return null;
  }

  const finalSession = rankSessions.finalizeSession(session.id);
  if (!finalSession) { console.warn(`⚠️ [${roomId}]: session đã finalize/xoá.`); return null; }
  console.log(`✅ [${roomId}]: đủ điều kiện chốt, tính ELO cho ${realPlayers.length} người.`);
  ...
  for (const [userId, resultData] of finalSession.resultMap) {
    ...
    const ign = userEloObj.ign || '';
    const displayName = ign ? `${ign} (${discordUsername})` : discordUsername;
    ...
    if (opponentElos.length === 0) {
      ...
      console.log(`ℹ️ ${displayName} [${userId}] test 1 mình, dùng ELO mặc định (${config.RANK_DEFAULT_ELO}).`);
    }
    ...
    eloStore.upsertElo(userId, getFullElo(userId), displayName)
      .then(() => console.log(`✅ ELO [${room.mode}] ${displayName} [${userId}]: ${userElo} → ${newElo}`))
      .catch((err) => console.error(`❌ Lỗi upsert ELO ${displayName}:`, err));
  ...
  if (publicChannelId) {
    const publicChannel = await client.channels.fetch(publicChannelId).catch((err) => {
      console.error(`❌ [${roomId}] Không fetch kênh:`, err.message);
      return null;
    });
    if (publicChannel) {
      ...
      const victoryText = winners.length > 0 ? winners.map((u, i) => formatEntry(u, i)).join('\n\n') : '> _Không có ai_';
      ...
      await publicChannel.send({ embeds: [embed] })
        ...
        .catch((err) => console.error(`❌ [${roomId}] Gửi kết quả THẤT BẠI:`, err.message));
    } else {
      console.warn(`⚠️ Không tìm thấy kênh sảnh chung: ${publicChannelId}.`);
    }
  } else {
    console.warn('⚠️ Chưa set PUBLIC_RESULT_CHANNEL_ID.');
  }
...
function loadImageHistory() {
  try { if (!fs.existsSync(HISTORY_FILE)) return []; return JSON.parse(fs.readFileSync(HISTORY_FILE, 'utf-8')); }
  catch { return []; }
}
...
function hashImageBuffer(buffer) { return crypto.createHash('sha256').update(buffer).digest('hex'); }
  ...
  if (searchChannel) {
    try {
      const recent = await searchChannel.messages.fetch({ limit: 50 }).catch(() => new Map());
        ...
        // ✅ FIX: title có thể có emoji prefix (📋) → dùng endsWith/includes
    ...
    if (resultChannelId) {
      ...
      if (resultChannel) {
        ...
        await resultChannel.send({
          content: `🎮 **${room.label}** [${room.mode}] đã bắt đầu!\nNgười chơi: ${playerMentions}\nKéo thả ảnh kết quả vào kênh này để nộp (hoặc dùng \`/submit-result phong:${room.id}\`). Hạn 45 phút.`
        });
      }
    }
...
client.on('messageCreate', async (message) => {
  try {
    console.log(`🔔 [messageCreate] author=${message.author.tag} channelId=${message.channelId} attachments=${message.attachments.size} isBot=${message.author.bot} isGuild=${!!message.guild}`);
    ...
    if (!session && submitterIsAdmin) {
      session = rankSessions.getLatestActiveSession();
      if (session && session.roomId === 'admin-test') {
        session = null;
      } else if (session) {
        adminTestMode = true;
    ...
    if (!session) {
      ...
      for (const r of getAllRankRooms()) {
        const f = rankSessions.wasRecentlyFinalized(r.id, message.author.id);
        if (f) { finalizedInfo = f; break; }
      }
      if (finalizedInfo) {
        const myEntry = finalizedInfo.players.find(([id]) => id === message.author.id);
        const myTeam = myEntry ? myEntry[1].team : null;
        const m = await message.reply({
          content: `✅ Trận này đã có kết quả từ teammate (team ${myTeam || '?'}). ELO của bạn đã được tính rồi — không cần gửi lại.`
        }).catch(() => null);
    ...
    const isFakeAdminSession = session.roomId === 'admin-test';
    ...
    if (session.resultMap.has(message.author.id)) {
      const m = await message.reply({ content: 'ℹ️ Bạn đã gửi kết quả rồi.' }).catch(() => null);
      if (m) setTimeout(() => m.delete().catch(() => {}), 15000);
      return;
    }

    const processingMsg = await message.reply({ content: '📥 Đang xử lý ảnh...' }).catch(() => null);
    const editOrIgnore = async (text) => { if (processingMsg) await processingMsg.edit(text).catch(() => {}); };

    let imageBuffer;
    try { imageBuffer = await downloadImageBuffer(attachment.url); }
    catch (e) { console.error('❌ Lỗi tải ảnh:', e.message); return editOrIgnore('❌ Không tải được ảnh.'); }

    const imageHash = hashImageBuffer(imageBuffer);

    if (!submitterIsAdmin && isImageHashUsedRecently(imageHash, message.author.id)) {
      return editOrIgnore('❌ Ảnh này đã dùng trong 90 ngày qua.');
    }

    if (config.LOG_CHANNEL_ID) {
      try {
        const logChannel = await client.channels.fetch(config.LOG_CHANNEL_ID);
        if (logChannel) await logChannel.send({ content: `📸 **${message.author.tag}** gửi ảnh cho **${session.roomId}**`, files: [attachment.url] });
      } catch {}
    }
    if (!submitterIsAdmin) addImageHistory(imageHash, attachment.url, message.author.id, session.roomId);

    let ocrText = '', overlayLines = [];
    try {
      const r = await ocrImageBuffer(imageBuffer);
      ocrText = r.text; overlayLines = r.overlayLines;
    } catch (err) {
      console.error('❌ Lỗi OCR:', err.message);
      return editOrIgnore(err.message === 'MISSING_OCR_API_KEY' ? '❌ Bot chưa có OCR key.' : '❌ Lỗi OCR.');
    }
    ...
    if (kdaMap.size === 0) return editOrIgnore('❌ Không tìm thấy KDA trong ảnh.');
    ...
    if (!senderKDA) {
      if (submitterIsAdmin) {
        return editOrIgnore(
          `❌ **Không tìm thấy tên bạn trong ảnh.**\n` +
          `Để test, bạn cần **đổi tên trong game** thành dạng \`<code>_<IGN>\` rồi chụp lại.\n` +
          `VD: IGN của bạn là \`NaNi\` → đổi thành \`1992-1_NaNi\` hoặc \`1992_NaNi\`.`
        );
      }
      return editOrIgnore('⚠️ Không tìm thấy tên của bạn trong ảnh → không tính ELO.');
    }

    const saved = rankSessions.addResult(session.id, message.author.id, {
      result,
    ...
    if (adminTestMode) {
      const myUpdate = Array.isArray(eloUpdates)
        ? eloUpdates.find(u => u.userId === message.author.id)
        : null;
      const delta = myUpdate ? myUpdate.newElo - myUpdate.oldElo : null;
      const deltaStr = delta === null ? '—' : `${delta >= 0 ? '+' : ''}${delta}`;
      const eloLine = myUpdate
        ? `• ELO [${session.mode}]: \`${myUpdate.oldElo} → ${myUpdate.newElo}\` (**${deltaStr}**)`
        : '• ELO: _(không có dữ liệu)_';

      await editOrIgnore(
        `🧪 **[ADMIN TEST]** Đã xử lý ảnh thành công.\n` +
        `• Chế độ: **${session.mode.toUpperCase()}** (tự nhận diện)\n` +
        `• Kết quả: **${result === 'win' ? 'Thắng' : 'Thua'}**\n` +
        `• KDA: ${senderKDA.kill}/${senderKDA.death}/${senderKDA.assist}\n` +
        eloLine
      );
      if (processingMsg) setTimeout(() => processingMsg.delete().catch(() => {}), 60000);
    } else {
      await editOrIgnore(`✅ Đã ghi nhận kết quả **${result === 'win' ? 'Thắng' : 'Thua'}**.`);
      if (processingMsg) setTimeout(() => processingMsg.delete().catch(() => {}), 30000);
    }
  } catch (err) {
    console.error('❗ Lỗi messageCreate (auto-submit ảnh):', err);
  }
});
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (err && (err.code === 40060 || err.code === 10062)) {
      console.warn(`⚠️ Interaction hết hạn hoặc đã ack (code ${err.code}) — bỏ qua.`);
      return;
    }
...
function wrapAdminEphemeralAutoDelete(interaction) {
  ...
  interaction.reply = async (options) => {
    try {
      ...
      if (err && (err.code === 40060 || err.code === 10062)) {
        console.warn(`⚠️ [wrapAdmin] reply bị bỏ qua (code ${err.code}).`);
        return null;
      }
      throw err;
    }
  ...
  interaction.followUp = async (options) => {
    try {
      ...
      if (err && (err.code === 40060 || err.code === 10062)) {
        console.warn(`⚠️ [wrapAdmin] followUp bị bỏ qua (code ${err.code}).`);
        return null;
      }
      throw err;
    }
...
async function handleSlashCommand(interaction) {
  ...
  if (DEFER_FIRST_COMMANDS.has(commandName)) {
    ...
    try {
      ...
      if (e && (e.code === 10062 || e.code === 40060)) {
        console.warn(`[${commandName}] deferReply bị bỏ qua (code ${e.code}) — interaction đã hết hạn.`);
      } else {
        console.error(`[${commandName}] deferReply thất bại:`, e.message);
      }
    }
    if (deferred) {
      interaction.reply = (opts) => interaction.editReply(opts).catch(() => null);
      interaction.followUp = () => Promise.resolve(null);
    } else {
      interaction.reply = () => Promise.resolve(null);
      interaction.followUp = () => Promise.resolve(null);
      interaction.editReply = () => Promise.resolve(null);
    }
  }

  if (commandName === 'restart') {
    if (!isAdmin(interaction)) return interaction.reply({ content: '❌ Chỉ admin.', ephemeral: true });
    await interaction.reply({ content: '🔄 Bot sẽ restart sau 2 giây...', ephemeral: true });
    console.log(`🔄 Admin ${interaction.user.tag} [${interaction.user.id}] yêu cầu restart.`);
    setTimeout(() => {
      console.log('🔄 Đang restart theo yêu cầu admin...');
      persistence.flushState(rooms, eloData);
      process.exit(1);
    }, 2000);
    return;
  ...
  if (commandName === 'ign-list') {
    ...
    if (all.length === 0) return interaction.reply({ content: 'ℹ️ Chưa có ai đăng ký IGN.', ephemeral: true });
  ...
  if (commandName === 'rank-top') {
    ...
    if (all.length === 0) return interaction.reply({ content: `ℹ️ Chưa có ai có ELO **${mode.toUpperCase()}**.`, ephemeral: true });
  ...
  if (commandName === 'register-ign') {
    ...
    eloStore.upsertElo(interaction.user.id, getFullElo(interaction.user.id), interaction.user.tag).catch(() => {});
    persistence.saveState(rooms, eloData);
  ...
  if (commandName === 'set-elo') {
    ...
    eloStore.upsertElo(targetUser.id, getFullElo(targetUser.id), targetUser.tag).catch(() => {});
    persistence.saveState(rooms, eloData);
  ...
  if (commandName === 'submit-result') {
    ...
    if (config.LOG_CHANNEL_ID) {
      try {
        const logChannel = await client.channels.fetch(config.LOG_CHANNEL_ID);
        if (logChannel) await logChannel.send({ content: `📸 **${interaction.user.tag}** gửi ảnh cho **${room.id}** (${room.label})`, files: [attachment.url] });
      } catch (err) {}
    }
    ...
    const fakeRoom = { players: new Map(session.players), code: session.code };
    const kdaMap = extractAllKDAResult(ocrText, fakeRoom, { skipCodeCheck: submitterIsAdmin, overlayLines });
    if (submitterIsAdmin) console.warn(`🧪 skipCodeCheck cho admin ${interaction.user.tag} [${interaction.user.id}]`);
...
async function handleButton(interaction) {
  ...
  if (customId.startsWith('submit_result_')) {
    ...
    return interaction.reply({
      content: `📷 **Gửi kết quả**\nCách 1: **Kéo thả ảnh** vào kênh <#${RANK_RESULT_CHANNEL_ID_ENV || 'rank-result'}> — bot tự xử lý.\nCách 2: Dùng lệnh kèm ảnh:\n\`\`\`/submit-result phong:${room.id}\`\`\``,
      ephemeral: true,
    });
  }
}
...
async function toggleReady(interaction, roomId) {
  ...
  try { await interaction.deferReply({ ephemeral: true }); }
  catch (err) { console.error('❗ toggleReady deferReply:', err.message); return; }
  ...
  return interaction.editReply({ content: t(interaction, player.ready ? '✅ Bạn đã sẵn sàng.' : '↩️ Bạn bỏ sẵn sàng.', player.ready ? '✅ Ready.' : '↩️ No longer ready.') + extra }).catch((err) => {
    console.error('❗ toggleReady editReply:', err.message);
  });
}
...
async function setTeam(interaction, roomId, team) {
  ...
  try { await interaction.deferReply({ ephemeral: true }); }
  catch (err) { console.error('❗ setTeam deferReply:', err.message); return; }
  ...
  return interaction.editReply({ content: team ? t(interaction, `✅ Đã chọn **Team ${team}**.`, `✅ Picked **Team ${team}**.`) : t(interaction, '✅ Bỏ chọn team.', '✅ Cleared team.') }).catch((err) => {
    console.error('❗ setTeam editReply:', err.message);
  });
}
